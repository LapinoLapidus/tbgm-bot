...
pub async fn prep_command(ctx: &Context, command: &ApplicationCommandInteraction) -> anyhow::Result<(), String> {
    let user = command.data.options.get(0).unwrap().resolved.as_ref().expect("No user object");
    let str = command.data.options.get(1).unwrap().resolved.as_ref().expect("No user object");
    ...
    util::give_rep(RepType::PlusRep, user.id.0 as i64, str, command.user.id.0 as i64).await.expect("F");
